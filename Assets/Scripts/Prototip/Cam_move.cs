...
public class Cam_move : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.position += Vector3.Scale(new Vector3 (0f, (_CamF.playerDist * Offset - transform.position.y)/20,  0f), transform.forward*-1) + new Vector3 (0f, 0f, _CamF.transform.position.z - transform.position.z - fixDist - (transform.position.y-35)*fixfixDist);
